"""Markdown renderer for audit retention reports."""
...
def _format_candidate(candidate: dict[str, Any]) -> str:
    details: list[str] = [
        ...
        f"family={candidate.get('report_family', 'unknown')}",
    ...
    details.append(f"retained_by_policy={'yes' if candidate.get('retained_by_policy') else 'no'}")
    details.append(f"newer_equivalent_exists={'yes' if candidate.get('newer_equivalent_exists') else 'no'}")
...
def _render_duplicate_reports(groups: list[dict[str, Any]]) -> list[str]:
...
def render_audit_retention_report(result: dict[str, Any], output_path: str | Path) -> None:
    """Persist a markdown audit retention report."""
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Total reports scanned: {summary.get('total_reports_scanned', 0)}",
    ...
    lines.extend(_render_candidate_section("Safe To Delete Now", result.get("safe_to_delete_candidates", [])))
    lines.extend(
    ...
    lines.extend(_render_group_section("Grouped By Family", result.get("grouped_by_family", []), "report_family"))
    ...
    lines.extend(_render_duplicate_reports(result.get("duplicate_report_groups", [])))
