...
class PythonImportGraphEngine(ImportGraphEngine):
    def build_graph(
        ...
        debug_print(debug, "ENTRYPOINTS RAW:", entrypoints)
        debug_print(debug, "MODULE MAP SAMPLE:", list(module_map.keys())[:5])
        debug_print(debug, "ENTRYPOINTS NORMALIZED:", normalized_eps)

        for entry in normalized_eps:
            if entry in graph:
                dfs(entry)

        debug_print(debug, "PY FILE COUNT:", len(all_paths))
        debug_print(
            debug,
            "GRAPH EDGE COUNT:",
            sum(len(neighbors) for neighbors in graph.values()),
        )
        debug_print(debug, "ENTRYPOINTS:", entry_modules)
